...
export default function SocialSharing({ city, className = '' }: SocialSharingProps) {
  ...
  const currentUrl = `https://edu.k2aihub.com/city/${city.slug}`;
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md p-4 ${className}`}>
      <h3 className="text-lg font-semibold text-gray-800 mb-3">Chia sẻ thông tin {city.name}</h3>
      ...
      <div className="mt-3 text-xs text-gray-500">Chia sẻ để giúp bạn bè tìm hiểu về {city.name}</div>
    </div>
  );
}
