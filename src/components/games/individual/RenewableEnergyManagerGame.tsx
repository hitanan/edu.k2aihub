...
import { Sun, Wind, Zap, Droplets, Leaf, Factory, AlertCircle } from 'lucide-react';
...
export default function RenewableEnergyManagerGame() {
  ...
  function calculateRenewableRatio(): number {
    ...
    Object.entries(grid.sources).forEach(([sourceId, count]) => {
      ...
      if (source && count > 0) {
        ...
        if (
          source.weatherDependency &&
          (source.type === 'solar' || source.type === 'wind' || source.type === 'hydro')
        ) {
  ...
  const nextTurn = () => {
    ...
    setTurn((prev) => prev + 1);
    addToLog(
      `💰 Thu nhập: ${income.toLocaleString()} - Chi phí: ${costs.toLocaleString()} = ${netIncome.toLocaleString()} đồng`,
    );
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-gray-800 rounded-xl text-white">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-8 rounded-xl max-w-2xl mx-4">
            ...
            <div className="space-y-4 text-gray-300">
              <p>
                • <strong>Mục tiêu:</strong> Xây dựng hệ thống năng lượng sạch và bền vững
              </p>
              <p>
                • <strong>Cân bằng:</strong> Đảm bảo sản xuất đủ để đáp ứng nhu cầu
              </p>
              <p>
                • <strong>Môi trường:</strong> Tối đa hóa năng lượng tái tạo, giảm phát thải
              </p>
              <p>
                • <strong>Thời tiết:</strong> Năng lượng tái tạo bị ảnh hưởng bởi điều kiện thời tiết
              </p>
              <p>
                • <strong>Hài lòng:</strong> Duy trì mức độ hài lòng của người dân {'>'} 50%
              </p>
            </div>
            <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className={`p-4 rounded-lg text-center ${energyBalance >= 0 ? 'bg-green-900' : 'bg-red-900'}`}>
          <div className="text-2xl font-bold">
            {energyBalance >= 0 ? '+' : ''}
            {energyBalance.toFixed(0)} MW
          </div>
        ...
        <div
          className={`p-4 rounded-lg text-center ${
            satisfactionLevel > 70 ? 'bg-green-900' : satisfactionLevel > 40 ? 'bg-yellow-900' : 'bg-red-900'
          }`}
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-300">
                Ánh sáng: {(currentWeather.sunlight * 100).toFixed(0)}% | Gió:{' '}
                {(currentWeather.windSpeed * 100).toFixed(0)}% | Mưa: {(currentWeather.rainfall * 100).toFixed(0)}%
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm text-gray-300">Lượt {turn}</div>
            <div className="text-sm text-gray-300">Phát thải: {emission.toFixed(1)} tCO₂/lượt</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">🏪 Cửa Hàng Năng Lượng</h2>
            ...
            {ENERGY_SOURCES.map((source) => {
              ...
              return (
                ...
                >
                  ...
                  {count > 0 && <div className="mb-2 text-sm text-blue-300">Sở hữu: {count} đơn vị</div>}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-700 p-6 rounded-lg">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">🌍 Tác Động Môi Trường</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="text-sm mb-2">Tổng phát thải CO₂: {totalCarbonEmission.toFixed(1)} tấn</div>
                <div
                  className={`text-sm ${
