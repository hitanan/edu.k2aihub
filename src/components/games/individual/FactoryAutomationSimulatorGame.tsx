...
export function FactoryAutomationSimulatorGame({ onComplete, timeLeft, onRestart }: GameProps) {
      ...
      setProductionLines(prevLines => {
        ...
        setSensors(prevSensors => {
          return prevSensors.map(sensor => {
            ...
            let value = 0;
            switch (sensor.type) {
              case 'temperature': value = line.temperature; break;
              case 'pressure': value = line.pressure; break;
              case 'quality': value = line.quality; break;
              case 'efficiency': value = line.efficiency; break;
            }
  ...
  }, [gameState, productionLines, qualityScore]);
  ...
  const toggleProductionLine = useCallback((lineId: string) => {
    setProductionLines(prev => prev.map(line => 
      line.id === lineId ? { ...line, isRunning: !line.isRunning } : line
    ));
  }, []);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-8 max-w-md w-full border border-blue-500/20 text-center">
          ...
          <div className="space-y-2 mb-6">
            ...
            <p className="text-gray-300">Tổng sản lượng: <span className="text-green-400 font-bold">{totalProduction.toFixed(1)}</span></p>
            <p className="text-gray-300">Chất lượng: <span className="text-yellow-400 font-bold">{qualityScore.toFixed(1)}%</span></p>
          </div>
          <button onClick={onRestart} className="bg-gradient-to-r from-blue-500 to-purple-500 text-white py-3 px-6 rounded-xl">
            Chơi lại
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
        ...
        <div className="lg:col-span-2 bg-black/20 backdrop-blur-sm rounded-xl p-6 border border-white/10">
          ...
          <div className="space-y-4">
            {productionLines.map(line => (
              <div key={line.id} className={`border rounded-lg p-4 ${line.hasIssue ? 'border-red-500 bg-red-500/10' : line.isRunning ? 'border-green-500 bg-green-500/10' : 'border-gray-500 bg-gray-500/10'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="text-gray-300">
                    Hiệu suất: <span className={line.efficiency > 70 ? 'text-green-400' : 'text-yellow-400'}>{line.efficiency.toFixed(1)}%</span>
                  </div>
                  <div className="text-gray-300">
                    Chất lượng: <span className={line.quality > 80 ? 'text-green-400' : 'text-yellow-400'}>{line.quality.toFixed(1)}%</span>
                  </div>
                  <div className="text-gray-300">
                    Nhiệt độ: <span className={line.temperature > 35 ? 'text-red-400' : line.temperature > 30 ? 'text-yellow-400' : 'text-green-400'}>{line.temperature.toFixed(1)}°C</span>
                  </div>
                  <div className="text-gray-300">
                    Áp suất: <span className={line.pressure > 1.8 ? 'text-red-400' : line.pressure > 1.5 ? 'text-yellow-400' : 'text-green-400'}>{line.pressure.toFixed(1)} bar</span>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-black/20 backdrop-blur-sm rounded-xl p-6 border border-white/10">
            ...
            <div className="space-y-3">
              ...
              <div className="text-gray-300">
                Dây chuyền hoạt động: <span className="text-blue-400 font-bold">{productionLines.filter(l => l.isRunning).length}/{productionLines.length}</span>
              </div>
              <div className="text-gray-300">
                Sự cố: <span className="text-red-400 font-bold">{productionLines.filter(l => l.hasIssue).length}</span>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 bg-black/20 backdrop-blur-sm rounded-xl p-4 border border-white/10">
        ...
        <p className="text-gray-300 text-sm">
          🎯 Quản lý dây chuyền sản xuất để tối ưu hiệu suất và chất lượng. 
          💡 Theo dõi cảm biến IoT và sửa chữa khi có sự cố. 
          ⚡ Đạt điểm số cao nhất trong thời gian cho phép!
        </p>
      </div>
    </div>
  );
