...
const RecyclingPlantManagerGame: React.FC<GameProps> = ({ onComplete, timeLeft, onRestart }) => {
  const [gamePhase, setGamePhase] = useState<
    'briefing' | 'waste-selection' | 'technology-selection' | 'sustainability-selection' | 'operation' | 'results'
  >('briefing');
  ...
  const calculatePlantPerformance = () => {
    ...
    const wasteComplexity =
      selectedWasteTypes.reduce((sum, w) => sum + (100 - w.difficulty), 0) / selectedWasteTypes.length;
    ...
    // Environmental score
    const wasteImpact =
      selectedWasteTypes.reduce((sum, w) => sum + w.environmental_impact, 0) / selectedWasteTypes.length;
    const techBenefit =
      selectedTechnologies.reduce((sum, t) => sum + t.environmental_benefit, 0) / selectedTechnologies.length;
    const sustainabilityScore =
      selectedFeatures.reduce((sum, f) => sum + f.environmental_score, 0) / Math.max(selectedFeatures.length, 1);
  ...
  const startOperation = () => {
    ...
    const operationComplexity =
      selectedWasteTypes.reduce((sum, w) => sum + w.difficulty, 0) / 10 +
  ...
  const getOverallScore = () => {
    ...
    const publicImage =
      selectedFeatures.reduce((sum, f) => sum + f.public_image_boost, 0) / Math.max(selectedFeatures.length, 1) / 5;

    return Math.round(
      plantResults.efficiency * 0.25 +
        plantResults.environmental * 0.3 +
        plantResults.profit * 0.2 +
        costEfficiency * 0.1 +
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-teal-900 to-emerald-900 flex items-center justify-center p-4">
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-8 max-w-4xl w-full border border-green-500/20">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-300 mb-6">Xây dựng và vận hành nhà máy tái chế hiện đại!</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white/10 rounded-xl p-6">
              ...
              <ul className="text-gray-300 space-y-2">
                <li>
                  • <span className="text-blue-400">Hiệu suất (25%)</span> - Khả năng xử lý
                </li>
                <li>
                  • <span className="text-green-400">Môi trường (30%)</span> - Tác động tích cực
                </li>
                <li>
                  • <span className="text-yellow-400">Lợi nhuận (20%)</span> - Bền vững kinh tế
                </li>
                <li>
                  • <span className="text-purple-400">Hiệu quả (25%)</span> - Chi phí và đa dạng
                </li>
              </ul>
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <div className="bg-gray-700 rounded-full h-1 mt-1">
                        <div className="bg-red-500 h-1 rounded-full" style={{ width: `${waste.difficulty}%` }}></div>
                      </div>
                    </div>
                    <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {SUSTAINABILITY_FEATURES.map((feature) => {
              ...
              const hasRequiredTech =
                ...
                feature.required_tech.every((techId) => selectedTechnologies.some((t) => t.id === techId));
              ...
              const canSelect =
                selectedFeatures.length < 4 && hasRequiredTech && currentCostWithoutFeature + feature.cost <= budget;
                    ...
                    <div className="mt-3 p-2 bg-red-500/20 rounded text-xs">
                      ...
                      <div className="text-gray-300">
                        {feature.required_tech
                          ...
                          .map((techId) => PROCESSING_TECHNOLOGIES.find((t) => t.id === techId)?.name)
                          .join(', ')}
                      </div>
                    </div>
                  )}
            ...
            <div className="bg-white/10 rounded-xl p-6">
              ...
              <div className="space-y-2 text-gray-300">
                ...
                {plantScore >= 80 && plantScore < 90 && (
                  <p>🌟 Nhà máy tái chế chất lượng! Đóng góp tích cực cho môi trường.</p>
                )}
                {plantScore >= 70 && plantScore < 80 && <p>✅ Nhà máy tái chế tốt! Cần cải thiện một số quy trình.</p>}
                {plantScore >= 60 && plantScore < 70 && (
                  <p>⚠️ Nhà máy cơ bản! Hãy đầu tư thêm công nghệ và tính năng.</p>
                )}
                ...
                <div className="mt-4 pt-4 border-t border-white/20">
                  ...
                  <p className="text-sm mt-2">
                    <strong>Lưu ý:</strong> Nhà máy tái chế thành công cần cân bằng giữa hiệu quả, môi trường và lợi
                    nhuận.
                  </p>
                </div>
              </div>
