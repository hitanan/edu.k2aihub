...
export default function DataDetective() {
  const gameData = EDUCATIONAL_GAMES_DATA.find((game) => game.id === 'data-detective');

  if (!gameData) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-900/20 to-purple-900/20">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-400 mb-4">Game Not Found</h1>
          <p className="text-gray-300">The Data Detective game could not be loaded.</p>
        </div>
      </div>
    );
  }
