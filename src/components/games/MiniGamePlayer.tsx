...
export function MiniGamePlayer({ game, onComplete, onExit }: MiniGameProps) {
      ...
      const timer = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            ...
            const finalScore = Math.round(score + timeBonus + streakBonus + perfectBonus);
  ...
  }, [currentGameState, timeLeft, isTimerActive, score, maxStreak, lives, startTime, game.id, onComplete, getTimeBonus]);
  ...
  if (currentGameState === 'menu') {
    ...
    const educationalGame = EDUCATIONAL_GAMES_DATA.find(g => g.id === game.id);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-8 max-w-2xl w-full border border-white/10">
          <div className="text-center mb-6">
            ...
            {educationalGame?.relatedLessons && educationalGame.relatedLessons.length > 0 && (
              <div className="mb-6 p-4 bg-gradient-to-r from-purple-900/30 to-blue-900/30 rounded-xl border border-purple-500/30">
                    ...
                    <div key={index} className="bg-black/20 rounded-lg p-3">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 text-left">
                          ...
                          <p className="text-gray-300 text-sm mt-1">{lesson.connection}</p>
                ...
                <div className="mt-3 text-xs text-gray-400 text-center">
                  💡 Chơi game này sẽ giúp bạn hiểu sâu hơn về các bài học trên
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">{game.difficulty} • {game.category}</p>
              </div>
            </div>

            <div className="flex items-center space-x-6">
              {/* Timer with visual warning */}
              <div className={`flex items-center space-x-1 ${timeLeft <= 30 ? 'text-red-400 animate-pulse' : timeLeft <= 60 ? 'text-yellow-400' : 'text-emerald-400'}`}>
...
function RoboticsNavigationGame({
    ...
    while (openSet.length > 0) {
      ...
      if (current.pos[0] === end[0] && current.pos[1] === end[1]) {
        ...
        while (node) {
          ...
          node = openSet.find(n => n.pos[0] === node.parent?.[0] && n.pos[1] === node.parent?.[1]) ||
          ...
          if (newPos[0] === maze.end[0] && newPos[1] === maze.end[1]) {
            ...
            if (currentLevel < gameData.mazes.length - 1) {
              setTimeout(() => {
                ...
                resetRobot();
                alert(`🎉 Hoàn thành cấp độ ${currentLevel + 1}! Điểm: ${levelScore}. Tiến tới cấp độ ${currentLevel + 2}!`);
              }, 1000);
            } else {
  ...
  const getCellStyle = (i: number, j: number) => {
    ...
    const isInAlgorithmPath = showAlgorithm && algorithmVisualization.path.some(([x, y]) => x === i && y === j);
    const isInOpenSet = showAlgorithm && algorithmVisualization.openSet.some(([x, y]) => x === i && y === j);
    const isInClosedSet = showAlgorithm && algorithmVisualization.closedSet.some(([x, y]) => x === i && y === j);

    const baseStyle = "w-8 h-8 border border-gray-600 flex items-center justify-center text-xs font-bold transition-all duration-300 transform hover:scale-110";
    
    if (isWall) return `${baseStyle} bg-gradient-to-br from-gray-800 to-gray-900 shadow-lg`;
    if (isPit) return `${baseStyle} bg-gradient-to-br from-red-900 to-red-800 shadow-lg`;
    if (isInAlgorithmPath) return `${baseStyle} bg-gradient-to-br from-yellow-400 to-orange-400 shadow-lg animate-pulse`;
    if (isInOpenSet) return `${baseStyle} bg-gradient-to-br from-green-400 to-green-500 shadow-lg`;
    if (isInClosedSet) return `${baseStyle} bg-gradient-to-br from-red-400 to-red-500 shadow-lg`;
    if (isInPath) return `${baseStyle} bg-gradient-to-br from-blue-400 to-blue-500 shadow-lg`;
    if (isStart || isEnd) return `${baseStyle} bg-gradient-to-br from-purple-500 to-indigo-500 shadow-lg`;
    ...
    <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-xl font-bold text-white">🤖 Điều hướng Robot 3D</h3>
        ...
        <p className="text-gray-300">
          Lập trình robot thông minh di chuyển từ vị trí xuất phát 🟢 đến đích 🎯 sử dụng thuật toán tối ưu
        </p>
          ...
          <div className="mt-3 p-3 bg-yellow-900/30 rounded-lg border border-yellow-500/50">
            <div className="text-yellow-300 text-sm">
              ...
              <div className="mt-1 grid grid-cols-3 gap-2 text-xs">
                ...
                <div>🔴 Closed Set: {algorithmVisualization.closedSet.length}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          ...
          {pathHistory.length > 0 && (
            <div className="mt-3 text-sm text-gray-400">
              📊 Đường đi: {pathHistory.length} bước | 
              🎯 Hiệu quả: {Math.round((1 - (pathHistory.length - maze.solution.length) / maze.solution.length) * 100)}%
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="mb-4">
            <h5 className="text-white text-sm mb-2">📋 Chuỗi lệnh ({commands.length}):</h5>
          ...
          <div className="mt-4 text-xs text-gray-400 space-y-1">
            <div>💡 <strong>Mẹo:</strong> Sử dụng &quot;🤖 A* Algorithm&quot; để xem đường đi tối ưu</div>
            <div>🎯 <strong>Mục tiêu:</strong> Hoàn thành với ít bước nhất để được điểm cao</div>
            <div>⚡ <strong>Điểm số:</strong> {score} | Cấp độ hiện tại: {currentLevel + 1}</div>
          </div>
        </div>
      </div>
    </div>
...
function ScratchAnimationGame({
  ...
  const runAnimation = () => {
    const requiredBlocks = ['when green flag clicked', 'forever', 'move 10 steps', 'if on edge, bounce'];
