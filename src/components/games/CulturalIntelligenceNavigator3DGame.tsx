    ...
    <>
      ...
      {nodes.map((node) => (
        <group key={node.id} position={node.position}>
          {/* Main Node */}
          <Sphere
            args={[node.explored ? 1.2 : 1, 32, 32]}
          ...
          <group rotation={[Math.PI / 2, 0, 0]}>
            <Cylinder
              args={[1.5, 1.5, 0.1, 32, 1, true]}
              position={[0, 0, 0]}
            >
              <meshBasicMaterial
                color={
                  node.difficulty === 1 ? '#10B981' :
                  node.difficulty === 2 ? '#F59E0B' : '#EF4444'
                }
                transparent
                opacity={0.3}
              />
          ...
          {/* Progress Indicators */}
          {node.explored && node.challenges.map((challenge, index) => {
            ...
            return (
              ...
              >
                <meshBasicMaterial
                  color={challenge.completed ? '#10B981' : '#64748B'}
                />
              </Box>
            );
          })}
          ...
          {explorationMode === 'exploring' && selectedNode === node.id && (
            <group>
              {Array.from({ length: 12 }).map((_, i) => {
                ...
                const radius = 2.5;
                return (
                  <Sphere key={i} args={[0.15, 8, 8]} position={[
                    Math.cos(angle) * radius,
                    Math.sin(angle) * radius,
      ...
      {connections.map(([from, to], index) => {
        ...
        return (
          <Cylinder
            ...
            position={midpoint}
            rotation={[
              Math.atan2(toNode.position[1] - fromNode.position[1],
                Math.sqrt(
                  Math.pow(toNode.position[0] - fromNode.position[0], 2) +
                  Math.pow(toNode.position[2] - fromNode.position[2], 2)
                )
...
const CulturalIntelligenceNavigator3DGame: React.FC<GameProps> = ({ onComplete, timeLeft, onRestart }) => {
  ...
  const exploreNode = useCallback((nodeId: string) => {
    ...
    setTimeout(() => {
      ...
      const completedPoints = completedChallenges
  ...
  }, [exploredNodes, nodes, onComplete]);

  const getChallengeIcon = (type: string) => {
    switch (type) {
      case 'communication': return <MessageCircle className="w-4 h-4" />;
      case 'etiquette': return <Star className="w-4 h-4" />;
