...
export default function SpaceExploration3DGameWrapper() {
  ...
  return (
    <div className="min-h-screen">
      {!gameState.completed ? (
        <SpaceExploration3DGame onComplete={handleGameComplete} timeLeft={gameState.timeLeft} onRestart={() => {}} />
      ) : (
        <div className="min-h-screen bg-gradient-to-br from-purple-900 to-indigo-900 flex items-center justify-center">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 max-w-md text-center text-white">
            <div className="text-6xl mb-4">{gameState.score > 1000 ? '🏆' : gameState.score > 500 ? '🥇' : '🎯'}</div>
            <h2 className="text-2xl font-bold mb-4">
              {gameState.score > 1000
                ? 'Phi hành gia xuất sắc!'
                : gameState.score > 500
                  ? 'Nhiệm vụ thành công!'
                  : 'Hoàn thành nhiệm vụ!'}
            </h2>
            <div className="bg-white/10 rounded-lg p-4 mb-6">
              ...
              <div className="mt-2 text-xs text-gray-400">Thời gian còn lại: {gameState.timeLeft}s</div>
            </div>
