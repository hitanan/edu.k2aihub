...
const EXCHANGE_SCENARIOS: ExchangeScenario[] = [
  {
    ...
    description:
      'Bạn cần đổi 50 triệu VNĐ sang EUR để du lịch châu Âu. Tỷ giá đang dao động, hãy chờ thời điểm tốt nhất!',
  ...
  },
  {
    ...
    description:
      'Công ty cần thanh toán 100,000 USD cho nhà cung cấp Mỹ. Hãy tìm thời điểm tỷ giá có lợi nhất trong tuần.',
    startingCurrency: 'VND',
    targetCurrency: 'USD',
    amount: 2450000000,
    timeLimit: 240,
  ...
  },
  {
    ...
    description:
      'Bạn muốn đầu tư 200 triệu VNĐ vào 3 thị trường khác nhau. Phân bổ giữa USD, EUR và JPY để tối ưu rủi ro.',
    startingCurrency: 'VND',
    targetCurrency: 'MIX',
    amount: 200000000,
    timeLimit: 300,
...
const CurrencyExchangePuzzleGame: React.FC<GameProps> = ({ onComplete, timeLeft, onRestart }) => {
  ...
  const triggerRandomEvent = () => {
    if (Math.random() < 0.3 && activeEvents.length < 2) {
      const availableEvents = MARKET_EVENTS.filter((event) => !activeEvents.some((active) => active.id === event.id));
  ...
  const getOverallScore = () => {
    ...
    const difficultyBonus =
      selectedScenario?.difficultyLevel === 'expert'
        ? 20
        : selectedScenario?.difficultyLevel === 'advanced'
          ? 15
          : selectedScenario?.difficultyLevel === 'intermediate'
            ? 10
            : 5;
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-slate-900 to-blue-900 flex items-center justify-center p-4">
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-8 max-w-4xl w-full border border-green-500/20">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-300 mb-6">Thành thạo nghệ thuật quy đổi ngoại tệ và kiếm lợi nhuận!</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white/10 rounded-xl p-6">
              ...
              <ul className="text-gray-300 space-y-2">
                <li>
                  • <span className="text-green-400">Lợi nhuận (40%)</span> - Tỷ suất sinh lời đạt được
                </li>
                <li>
                  • <span className="text-blue-400">Quản lý rủi ro (30%)</span> - An toàn giao dịch
                </li>
                <li>
                  • <span className="text-yellow-400">Timing (30%)</span> - Chọn thời điểm tối ưu
                </li>
                <li>
                  • <span className="text-purple-400">Bonus</span> - Độ khó scenario
                </li>
              </ul>
            </div>
          </div>
                    ...
                    <div className="text-right">
                      ...
                      <div
                        className={`text-xs px-2 py-1 rounded mt-1 ${
                          scenario.marketCondition === 'stable'
                            ? 'bg-blue-500/20 text-blue-400'
                            : scenario.marketCondition === 'volatile'
                              ? 'bg-yellow-500/20 text-yellow-400'
                              : scenario.marketCondition === 'crisis'
                                ? 'bg-red-500/20 text-red-400'
                                : 'bg-green-500/20 text-green-400'
                        }`}
                      >
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex justify-center items-center gap-4 text-lg">
              ...
              <span className="text-blue-400">
                {selectedScenario?.amount.toLocaleString()} {selectedScenario?.startingCurrency} →{' '}
                {selectedScenario?.targetCurrency}
              </span>
            </div>
          </div>
            ...
            <div className="bg-white/10 rounded-xl p-6">
              ...
              {exchangeResult ? (
                ...
                <div className="space-y-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white mb-2">{timeRemaining}s</div>
                  ...
                  <div className="text-xs text-gray-400 text-center">Đợi thời điểm tốt hoặc thực hiện ngay</div>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-3">
                {activeEvents.map((event) => (
                  <div key={event.id} className="bg-yellow-500/20 rounded-lg p-4">
                    ...
                    <div className="text-xs text-gray-400 mt-2">Ảnh hưởng: {event.affectedCurrencies.join(', ')}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-8 max-w-4xl w-full border border-green-500/20">
              ...
              {exchangeResult && (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-yellow-400 font-mono">{exchangeResult.exchangeRate.toFixed(4)}</span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`font-bold ${exchangeResult.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {exchangeResult.profit >= 0 ? '+' : ''}
                      {exchangeResult.profit.toFixed(2)}%
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white/10 rounded-xl p-6 mb-8">
            ...
            <div className="text-gray-300">
              {overallScore >= 90 && (
                <p>🏆 Trader xuất sắc! Bạn có khả năng phân tích thị trường và timing tuyệt vời.</p>
              )}
              {overallScore >= 80 && overallScore < 90 && (
                <p>🌟 Trader giỏi! Bạn hiểu rõ về thị trường ngoại tệ và quản lý rủi ro tốt.</p>
              )}
              {overallScore >= 70 && overallScore < 80 && (
                <p>✅ Trader có tiềm năng! Cần cải thiện kỹ năng phân tích và timing.</p>
              )}
              {overallScore >= 60 && overallScore < 70 && (
                <p>⚠️ Trader mới! Hãy học thêm về phân tích kỹ thuật và quản lý rủi ro.</p>
              )}
              {overallScore < 60 && <p>❌ Cần luyện tập nhiều hơn về giao dịch ngoại tệ và quản lý tài chính.</p>}

              <div className="mt-4 pt-4 border-t border-white/20">
                <p className="text-sm">
                  <strong>Gợi ý:</strong> Thị trường ngoại tệ rất nhạy cảm với tin tức kinh tế và địa chính trị. Hãy
                  luôn cập nhật thông tin và có chiến lược quản lý rủi ro rõ ràng.
                </p>
              </div>
            </div>
          </div>
