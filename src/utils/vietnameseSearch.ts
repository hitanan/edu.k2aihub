 ...
 */
export function vietnameseTextMatch(
  searchTerm: string,
  targetText: string,
): boolean {
 ...
 */
export function enhancedVietnameseSearch(
  searchTerm: string,
  targetText: string,
): boolean {
  if (!searchTerm || !targetText) return false;
  ...
  const vietnameseKeywords =
    englishToVietnameseKeywords[normalizedSearch] || [];
  if (
    vietnameseKeywords.some((keyword) =>
      normalizedTarget.includes(normalizeVietnameseText(keyword)),
    )
  ) {
    return true;
  }
  ...
  if (searchWords.length > 1) {
    return searchWords.some((word) => {
      ...
      return (
        keywords.some((keyword) => normalizedTarget.includes(keyword)) ||
 ...
 */
export function searchModulesVietnamese<
  T extends {
    title: string;
    description: string;
    subtitle?: string;
...
>(modules: T[], searchTerm: string): T[] {
  ...
  return modules.filter((module) => {
    ...
    if (
      enhancedVietnameseSearch(searchTerm, module.title) ||
    ...
    if (
      module.features?.some((feature) =>
        enhancedVietnameseSearch(searchTerm, feature),
      )
    ) {
      return true;
    }
 ...
 */
export function highlightVietnameseSearch(
  text: string,
  searchTerm: string,
): string {
