...
function parseLessonsFromFile(fileContent, moduleName) {
  try {
    ...
    const lessonRegex = /\{\s*(?:[^{}]|\{[^{}]*\})*id:\s*['"`]([^'"`]+)['"`](?:[^{}]|\{[^{}]*\})*title:\s*['"`]([^'"`]*?)['"`](?:[^{}]|\{[^{}]*\})*?\}/gs;
...
async function searchYouTubeVideo(query, apiKey) {
  try {
    ...
    const videos = data.items.filter(item => {
      ...
      const isEducationalChannel = educationalChannels.some(channel_name => 
        channel.includes(channel_name)
      );
