...
import { Clock, Target, User, Play, ChevronLeft, ChevronRight, TestTube, CheckCircle, Youtube, Lightbulb } from 'lucide-react'
import { PageProps } from '@/types'

// Generate static params
export async function generateStaticParams() {
  return stemLessons.map((lesson) => ({
    lessonId: lesson.id,
  }))
}
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!lesson) {
    return {
      ...
      description: createDescription('Bài học STEM này không tồn tại.')
    }
  }
  ...
  return {
    ...
    keywords: `STEM, ${lesson.title}, science, technology, engineering, mathematics, ${lesson.difficulty}, ${lesson.category}`,
  }
}
...
export default async function STEMLessonPage({ params }: PageProps) {
  ...
  const currentIndex = stemLessons.findIndex((l) => l.id === lessonId)
  const previousLesson = currentIndex > 0 ? stemLessons[currentIndex - 1] : null
  const nextLesson = currentIndex < stemLessons.length - 1 ? stemLessons[currentIndex + 1] : null

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'science': return <TestTube className="w-5 h-5" />
      case 'technology': return <Target className="w-5 h-5" />
      case 'engineering': return <User className="w-5 h-5" />
      case 'mathematics': return <Play className="w-5 h-5" />
      case 'arts': return <Lightbulb className="w-5 h-5" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-8">
          <div className="flex items-center space-x-3 text-sm">
            ...
            <Link href="/learning/stem" className="text-pink-400 hover:text-pink-300 transition-colors">
              STEM Education
            </Link>
            <span className="text-gray-400">/</span>
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <span className="text-pink-400 font-medium capitalize">{lesson.category}</span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white/10 rounded-lg p-4 text-center">
                  ...
                  <div className="text-sm text-gray-300">Thí nghiệm</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              <h2 className="text-2xl font-bold text-white mb-6">Thí nghiệm thực hành</h2>
                        ...
                        <ol className="space-y-2">
                          {exercise.steps.map((step, stepIndex) => (
                            <li key={stepIndex} className="text-gray-300 text-sm">
                              <span className="font-semibold text-blue-300">{stepIndex + 1}.</span> {step}
                            </li>
                          ))}
                        </ol>
                      </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              ...
              <ul className="space-y-3 text-sm text-gray-300">
                <li>• Quan sát kỹ hiện tượng</li>
                <li>• Ghi chép chi tiết</li>
                <li>• An toàn khi thí nghiệm</li>
                <li>• Tư duy phản biện</li>
                <li>• Kết nối với thực tế</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mt-12 pt-8 border-t border-white/20">
          {previousLesson ? (
            <Link 
              href={`/learning/stem/${previousLesson.id}`}
          ...
          {nextLesson && (
            <Link 
              href={`/learning/stem/${nextLesson.id}`}
              className="flex items-center space-x-3 bg-white/10 hover:bg-white/20 rounded-lg p-4 transition-all duration-200"
            >
