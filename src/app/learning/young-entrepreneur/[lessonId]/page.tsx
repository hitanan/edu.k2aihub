...
import {
  youngEntrepreneurLessons,
  YoungEntrepreneurLessonData,
} from '@/data/young-entrepreneur';
...
export default async function YoungEntrepreneurLessonPage({
  params,
}: PageProps) {
  const config: LessonPageConfig<YoungEntrepreneurLessonData> = {
    moduleName: 'young-entrepreneur',
    moduleTitle: 'Young Entrepreneur Bootcamp',
    ...
    getFieldValue: (lesson) => {
      ...
      if (lesson.vietnameseMarketFocus)
        return lesson.vietnameseMarketFocus.join(', ');
    ...
    sidebarContent: (lesson) => (
      <div className="space-y-6">
        ...
        {lesson.vietnameseMarketFocus &&
          lesson.vietnameseMarketFocus.length > 0 && (
