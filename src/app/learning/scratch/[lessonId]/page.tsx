...
export default async function ScratchLessonPage({ params }: PageProps) {
  ...
  const previousLesson =
    currentIndex > 0 ? scratchLessons[currentIndex - 1] : null;
  const nextLesson =
    currentIndex < scratchLessons.length - 1
      ? scratchLessons[currentIndex + 1]
      : null;
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white font-semibold">
                    {currentIndex + 1}/{scratchLessons.length}
                  </span>
                </div>
                <div className="w-full bg-gray-700 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${((currentIndex + 1) / scratchLessons.length) * 100}%`,
                    }}
                  ></div>
                </div>
                <div className="text-xs text-gray-400">
                  {Math.round(((currentIndex + 1) / scratchLessons.length) * 100)}% hoàn thành
                </div>
              </div>
              
              {/* Lesson List */}
              <div className="mt-4 space-y-2 max-h-64 overflow-y-auto">
                {scratchLessons.map((lessonItem, index) => (
                  <Link
                    key={lessonItem.id}
                    href={`/learning/scratch/${lessonItem.id}`}
