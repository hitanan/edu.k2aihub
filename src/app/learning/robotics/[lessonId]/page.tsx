...
import { Clock, Target, User, Play, ChevronLeft, ChevronRight, Bot, Settings, Lightbulb, CheckCircle, Youtube } from 'lucide-react'
...
export default async function RoboticsLessonPage({ params }: PageProps) {
  ...
  const previousLesson = currentIndex > 0 ? roboticsLessons[currentIndex - 1] : null
  const nextLesson = currentIndex < roboticsLessons.length - 1 ? roboticsLessons[currentIndex + 1] : null

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'hardware': return <Settings className="w-5 h-5" />
      case 'programming': return <Bot className="w-5 h-5" />
      case 'projects': return <Play className="w-5 h-5" />
      case 'theory': return <Target className="w-5 h-5" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-8">
          <div className="flex items-center space-x-3 text-sm">
            ...
            <Link href="/learning/robotics" className="text-purple-400 hover:text-purple-300 transition-colors">
              Robotics
            </Link>
            <span className="text-gray-400">/</span>
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <span className="text-purple-400 font-medium capitalize">{lesson.category}</span>
              </div>
                  ...
                  <div key={index} className="border border-white/20 rounded-lg p-6">
                    ...
                    {exercise.code && (
                      <div className="bg-black/50 rounded-lg p-4">
                        <h4 className="text-gray-300 font-semibold mb-2">💡 Code mẫu:</h4>
